# Runtime Tcl commands to interact with - hwaccel

# Sourcing design address info tcl
set bd_path [get_property DIRECTORY [current_project]]/[current_project].srcs/[current_fileset]/bd
source ${bd_path}/hwaccel_include.tcl
...
# Test: axi_lite
# Create a write transaction at axi_lite_addr address
create_hw_axi_txn w_axi_lite_addr [get_hw_axis $jtag_axi_master] -type write -address $axi_lite_addr -data $wdata_1
# Create a read transaction at axi_lite_addr address
create_hw_axi_txn r_axi_lite_addr [get_hw_axis $jtag_axi_master] -type read -address $axi_lite_addr
# Initiate transactions
run_hw_axi r_axi_lite_addr
run_hw_axi w_axi_lite_addr
run_hw_axi r_axi_lite_addr
set rdata_tmp [get_property DATA [get_hw_axi_txn r_axi_lite_addr]]
# Compare read data
if { $rdata_tmp == $wdata_1 } {
	puts "Data comparison test pass for - axi_lite"
} else {
	puts "Data comparison test fail for - axi_lite, expected-$wdata_1 actual-$rdata_tmp"
	inc ec
}
...
# Test: axim_in
set wdata_axim_in [get_init_data 0]
create_hw_axi_txn w_axim_in_addr [get_hw_axis $jtag_axi_master] -type write -address $axi_gpio_out_addr -data ${wdata_axim_in}
create_hw_axi_txn r_axim_in_addr [get_hw_axis $jtag_axi_master] -type read -address $axi_gpio_in_addr 
# Initiate transactions
run_hw_axi r_axim_in_addr
run_hw_axi w_axim_in_addr
run_hw_axi r_axim_in_addr
set rdata_tmp [get_property DATA [get_hw_axi_txn r_axim_in_addr]]
set DE [ get_done_and_error_bit $rdata_tmp 2 0 ]
# Compare read data
if { $DE == 01 } {
	puts "Data comparison test pass for - axim_in"
} else {
	puts "Data comparison test fail for - axim_in, rdata-$rdata_tmp expected-01 actual-$DE"
	inc ec
}

# Test: axim_out
set wdata_axim_out [get_init_data 1]
create_hw_axi_txn w_axim_out_addr [get_hw_axis $jtag_axi_master] -type write -address $axi_gpio_out_addr -data ${wdata_axim_out}
create_hw_axi_txn r_axim_out_addr [get_hw_axis $jtag_axi_master] -type read -address $axi_gpio_in_addr 
# Initiate transactions
run_hw_axi r_axim_out_addr
run_hw_axi w_axim_out_addr
run_hw_axi r_axim_out_addr
set rdata_tmp [get_property DATA [get_hw_axi_txn r_axim_out_addr]]
set DE [ get_done_and_error_bit $rdata_tmp 2 1 ]
# Compare read data
if { $DE == 01 } {
	puts "Data comparison test pass for - axim_out"
} else {
	puts "Data comparison test fail for - axim_out, rdata-$rdata_tmp expected-01 actual-$DE"
	inc ec
}
